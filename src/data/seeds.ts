...
// Seeds now start empty and are managed per-user via the vault store (localStorage).
// Each person who uses the app builds their own vault from scratch.
...
export const VAULT_TYPE_TOTALS = (["Feminized", "Regular", "Autoflower", "Unknown Photo"] as const).map(
  (type) => ({
    type,
    total: 0,
    strains: 0,
