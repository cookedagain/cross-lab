...
export const VaultProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setSeedCounts(countResult.success ? { ...DEFAULT_SEED_COUNTS, ...countResult.data } : { ...DEFAULT_SEED_COUNTS });
  ...
  useEffect(() => {
    if (storageReady && isSecureStorageUnlocked()) void writeSecure(INVENTORY_STORAGE_KEY, seedCounts);
  }, [seedCounts, storageReady]);

  useEffect(() => {
    if (storageReady && isSecureStorageUnlocked()) void writeSecure(MULTIPASS_STORAGE_KEY, multipass);
  }, [multipass, storageReady]);

  useEffect(() => {
    if (storageReady && isSecureStorageUnlocked()) void writeSecure(INCOMING_ARRIVALS_STORAGE_KEY, incomingArrivedIds);
  }, [incomingArrivedIds, storageReady]);

  useEffect(() => {
    if (storageReady && isSecureStorageUnlocked()) void writeSecure(LOTS_STORAGE_KEY, lots);
  }, [lots, storageReady]);
  ...
  const getSeedCount = (seed: Seed) => seedCounts[seed.id] ?? 0;
